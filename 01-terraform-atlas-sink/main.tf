...
locals {
  ...
  atlas_sink_uri = replace(
    mongodbatlas_advanced_cluster.CEP.connection_strings.standard_srv,
    "mongodb+srv://",
...
resource "confluent_connector" "datagen" {
 ...
 resource "confluent_connector" "mongodb_atlas_sink" {
  ...
  config_nonsensitive = {
    ...
    "connection.host"     = trimprefix(mongodbatlas_advanced_cluster.CEP.connection_strings.standard_srv, "mongodb+srv://")
...
resource "mongodbatlas_stream_processor" "unique_tickers" {
  ...
  pipeline = templatefile("${path.module}/pipelines/unique_tickers_json.tftpl", {
    ...
    connection_name   = mongodbatlas_stream_connection.atlas_connection.connection_name
    db_name           = var.stream_db_name
    input_collection  = var.input_collection
